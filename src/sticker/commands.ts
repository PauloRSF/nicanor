...
import { gifToSticker, imageToSticker } from "../_lib/whatsapp/media.js";
import type { Context } from "../messages/types.js";
import { Sticker, UnsavedSticker } from "./index.js";

type WithRequired<T, K extends keyof T> = T & { [P in K]-?: T[P] };

type SaveStickerParams = WithRequired<
  Omit<CommandParams<Context>, "args">,
  "sticker"
>;
...
export async function saveSticker({
  ...
  const sticker = await new UnsavedSticker({
    userId,
    data: stickerBytes,
  }).save();
  ...
  const existingTagsMessage =
    sticker.tags.length > 0
      ? `\n\nTags atuais: ${sticker.tags.join(", ")}`
      : "";

  await sendText(
    `Figurinha recebida!${existingTagsMessage}\n\nUse *!tag (ou !t) tag1 tag2 ...* para marcá-la.`,
  );
}

type SaveImageAsStickerParams = WithRequired<
  Omit<CommandParams<Context>, "args">,
  "image"
>;
...
export async function saveImageAsSticker({
  ...
  await sendText(
    `Imagem recebida e convertida para figurinha!${existingTagsMessage}`,
  );
  await sendSticker(stickerBytes);
  await sendText(`Use *!tag (ou !t) tag1 tag2 ...* para marcá-la.`);
}

type SaveGifAsStickerParams = WithRequired<
  Omit<CommandParams<Context>, "args"> & { gifInputExt?: "mp4" | "gif" },
  "gif"
>;
...
export async function saveGifAsSticker({
  ...
  await sendText(
    "GIF recebido, vou converter para figurinha... Isso pode levar alguns segundos.",
  );

  const stickerBytes = await gifToSticker(userId, gif, gifInputExt);
  ...
  await sendText(`GIF convertido para figurinha!${existingTagsMessage}`);
  await sendSticker(stickerBytes);
  await sendText(`Use *!tag (ou !t) tag1 tag2 ...* para marcá-la.`);
}

export async function tagSticker({
  args,
  logger,
  userId,
  sendText,
  ...
  if (!sticker) {
    return await sendText(
      "Nenhuma figurinha encontrada. Envie uma figurinha primeiro.",
    );
  }
...
export async function searchStickers({
  ...
  if (results.length === 0) {
    return await sendText(
      `Nenhuma figurinha encontrada para as tags "${args.join(", ")}"`,
    );
  }

  await sendText(
    `${results.length} figurinha(s) encontrada(s) para as tags "${args.join(", ")}"`,
  );
    ...
    try {
      if (!(await sticker.hasFile())) {
        logger.warn(
          ...
          "Sticker file missing in storage, skipping",
        );
        continue;
      }
      ...
      if (bytes.length === 0) {
        logger.warn(
          { stickerId: sticker.id },
          "Sticker file is empty, skipping",
        );
        continue;
      }

      logger.info(
        { stickerId: sticker.id, stickerByteLength: bytes.length },
        "Sending sticker",
      );
